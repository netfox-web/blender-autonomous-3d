(() => {
  ...
  function drawPending() {
    $('batch-pending').innerHTML=pending.map((s,i)=>`<li>${esc(s.sku)} · ${scenes[s.scene]} · ${s.placements.length} 個貼圖面 <button type="button" class="secondary" data-remove="${i}">移除</button></li>`).join('');
  ...
  async function history() {
    ...
    $('batch-history').innerHTML=d.items.map(row=>{
      ...
      return `<article class="variant-card"><h4>${esc(row.sku)}</h4><p>${esc(scenes[row.scene]||row.scene)} · 母版第 ${esc(row.sourceRevision)} 版</p>`+
  ...
  $('batch-add').onclick=safe(async()=>{
    ...
    if(chosen.some(scene=>pending.some(s=>s.sku===sku&&s.scene===scene)))throw Error('此款式名稱與背景已在清單中；換圖時請使用不同款式名稱');
    pending.push(...chosen.map(scene=>({sku,scene,placements:structuredClone(placements)})));
