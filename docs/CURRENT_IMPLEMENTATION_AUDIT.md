...
| Evidence Completeness Structure & Truncation Gate (Round 5 Blocker A) | REAL_LOGIC | `services/supervisor/models.py` & `ai_adapter.py`: `EvidenceSectionCompleteness` tracks `path`, `ref_sha`, `original_chars`, `supplied_chars`, `truncated`, `sha256`, `critical`, and chunk coverage. If any critical section is truncated without full coverage, deterministic gate halts with `CHANGES_REQUIRED`. Engine independently asserts all diff changed files appear in manifest. |
| Window B1 Exact 6-Trailer & Blob Identity & Digest Authority (Round 6 Blocker A) | REAL_LOGIC | `services/supervisor/engine.py`: Instruction candidate adoption unconditionally requires all 6 trailers (`Reviewed-Code-Sha`, `Reviewed-Docs-Sha`, `Reviewed-Instruction-Sha`, `Reviewed-Evidence-Id`, `Supervisor-Decision`, `Supervisor-Review-Id`); remote blob fetch fails closed on 404/empty/timeout; remote blob requires complete 6-identity marker; durable SHA256 digest persisted before write and asserted on adoption; staged commits verified before adoption; multi-candidate fails closed. |
| Live Commit Body Framing & 6-Trailer Preservation (Round 7 Blocker A) | REAL_LOGIC | `services/supervisor/github_client.py`: `get_commits_since()` uses ASCII Record Separator (`\x1e`) and Unit Separator (`\x1f`) framing (`--pretty=format:%x1e%H%x1f%an%x1f%B`) to preserve full raw commit body with newlines, pipes (`|`), and all 6 trailers. Remote compare API fallback parses full `commit.message`. Real repo integration test verified. |
| Fail-Closed Candidate Discovery & Staged Isolation (Round 7 Blocker B) | REAL_LOGIC | `services/supervisor/github_client.py` & `engine.py`: Raises typed `GitHubVerificationError` on git/API discovery failure; engine distinguishes zero commits from discovery errors; staged commits and candidate enumeration failures halt review and strictly prevent creating duplicate instruction commits. |
| Window B2 REST Fallback Pagination & Latest Comments (Round 7 Blocker C) | REAL_LOGIC | `services/supervisor/github_client.py`: `_parse_link_header()` handles RFC 5988 `Link` pagination; fetches `rel="last"` and `rel="prev"` to reliably retrieve latest comments in chronological order; fail-closed on fetch errors prevents duplicate comments on Issue #1. |
| Remote GitHub API Ref Normalization & Compare Fallback (Round 8 Blocker A) | REAL_LOGIC | `services/supervisor/github_client.py`: `_normalize_ref_for_api()` strips `origin/` prefix from branch refs in GitHub compare API queries (`compare/{base}...{api_head}`), preventing HTTP 404 errors; raises `GitHubVerificationError` on non-200 HTTP statuses; full 6-trailer preservation verified via real `GitHubClient` tests. |
| Mandatory Real Blender Acceptance vs Product Truth Isolation (Round 6 Blocker B) | REAL_LOGIC | `services/supervisor/engine.py`: `docs/REAL_E2E_ACCEPTANCE.md @ DOCS_SHA` is strictly mandatory when `real_blender=True and not used_mock`. 404/empty/timeout fails closed before AI provider call. Product Truth acceptance is strictly auxiliary and cannot substitute. |
| Authoritative Changed Files & Split Diff Ranges (Round 6 Blocker C) | REAL_LOGIC | `services/supervisor/github_client.py` & `engine.py`: `get_changed_files_between()` queries `git diff --name-status` / GitHub compare API; manifest validates against git authority (detects omitted, phantom, status spoof, rename mismatch); diff split into `CODE DIFF` (`INSTRUCTION_SHA..CODE_SHA`) and `DOCS DIFF` (`CODE_SHA..DOCS_SHA`) with contamination prevention. |
| Strict Parser Boundary DOCS_CI_RUN_ID & Provider Request Audit (Round 6 Blocker D) | REAL_LOGIC | `services/supervisor/models.py` & `ai_adapter.py`: Strict validation and parsing require positive numeric `DOCS_CI_RUN_ID` before GitHub API calls (no legacy substitution); provider responses extract request IDs into audit trail without leaking secrets. |
| Configurable SUPERVISOR_AI_MODEL & Live Config Validation (Round 5 Blocker D) | REAL_LOGIC | `services/supervisor/config.py` & `ai_adapter.py`: In live mode, `SUPERVISOR_AI_MODEL` is mandatory, non-empty, and prefix-validated against provider (`gpt-`, `o1`, `o3`, `claude-`, `gemini-`). Incompatible combinations fail at startup. |
...
| 72-Scenario Supervisor Test Suite | REAL_LOGIC | `tests/test_supervisor.py`: 72 comprehensive tests covering webhook, signature, idempotency, CI verification, provider dispatch, 4 reviewed identities, prompt metadata, crash windows, preflight, policy guardrails, evidence completeness, fail-closed pinned fetch, Window B1 6-trailer matching, blob digest authority, mandatory Real Blender acceptance, authoritative diff ranges, strict DOCS CI parser boundary, provider request ID audit, real git log trailer preservation, fail-closed candidate discovery, REST pagination, compare API ref normalization, and Window B2 exactly-once across all decisions |
...
| Door Articulation Transforms & Worker Evidence (Blocker A) | REAL_LOGIC / REAL | Physical 3D door hinges rotated to 75.0° around pivot in Blender scene when OPEN, restored to neutral; observed worker evidence strictly compared against canonical expectation (not replaced by recomputed state); mismatched transforms/job fail closed |
| Real DAM Asset Provenance Binding (Blocker B) | REAL_LOGIC | Fail-closed real worker artifact provenance in DAM metadata; no synthetic dimensions or empty bytes; worker evidence bound across all commerce views including DIMENSION_FRONT |
| Dimension Asset Authority & Rasterization (Blocker C) | REAL_LOGIC | Front dimension image overlay labels derived exclusively from `CabinetSpec` mm; deterministic pure-Python bitmap font rasterization on DIMENSION_FRONT; independent QA gate validates pixel overlay metadata against engineering mm; no CV inference; tampering fails closed |
...
| 959–960 Re-Gate R1 acceptance | REAL_LOGIC + REAL stills | `docs/PRODUCT_CONTENT_FACTORY_ACCEPTANCE.md`; generation `57f72869-a017-415f-bf66-7df55824d486`; CODE `4406119`; `usedMock=false`; `productContentFactoryLogicReady=true`; `realCommerceRenderPackReady=true`; `liveGenerativeCommerceReady=false`; `commercialAssetProductionReady=false` |
