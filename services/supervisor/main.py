...
from services.supervisor.security import SecurityError, verify_github_signature
...
def create_app(config: Optional[SupervisorConfig] = None) -> FastAPI:
    ...
    # Store instances in app state for testing and route access
    ...
    async def github_webhook(
        ...
        if delivery_id and state_mgr.has_seen_delivery(delivery_id):
        ...
        if delivery_id:
            state_mgr.record_delivery(delivery_id, x_github_event or "unknown")
        ...
        if event_name == "issue_comment":
            ...
            # Informational handling / status tracking for CI and push events
